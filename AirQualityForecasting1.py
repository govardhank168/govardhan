...
plt.title(rmse(ts['pm25'], results_AR.fittedvalues))
...
plt.title(rmse(ts['pm25'], predictions_ARIMA))

# Fit a SARIMAX(3, 0, 2)x(2, 1, 0, 12) on the training set 

model = sm.tsa.statespace.SARIMAX(ts['pm25'], trend='c', order=(3,0,2))
...
plt.title(rmse(ts['pm25'], results.fittedvalues))
#Code : Predictions of ARIMA Model against the test set (filter_none,edit, play_arrow,brightness_4)

#ts_log=ts_log.reset_index('date')

train = ts.iloc[:len(ts)-36] 
test = ts.iloc[len(ts)-36:]
...
forecast = results.predict(start = len(train),  
                          ...
                          type = 'pm25').rename('Forecast') 

forecast.to_csv('Forecast.csv')
# Plot the forecast values 
train['pm25'].plot(figsize = (12, 5), legend = True) 
forecast.plot(legend = True) 


# Calculate root mean squared error 
rmse(test["pm25"], forecast) 
ts['predicted'] = predict_ci
ts.hist(figsize=(16, 20), bins=50, xlabelsize=8, ylabelsize=8)
