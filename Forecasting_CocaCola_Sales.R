...
CocaCola<-read.csv("D:/DS/Assignments/Assignments/Forecasting/CocaCola_Sales_Rawdata.csv")
View(CocaCola) # Seasonality 12 X 
windows()
plot(CocaCola$Sales,type="o")
# So creating 12 dummy variables 

X<- data.frame(outer(rep(month.abb,length = 42), month.abb,"==") + 0 )# Creating dummies for 12 X
  View(X)
...
Add_sea_Linear_model<-lm(Sales~Jan+Feb+Mar+Apr+May+Jun+Jul+Aug+Sep+Oct+Nov+Dec,data=train)
...
Add_sea_Quad_model<-lm(Sales~Jan+Feb+Mar+Apr+May+Jun+Jul+Aug+Sep+Oct+Nov+Dec,data=train)
...
multi_sea_model<-lm(Sales~Jan+Feb+Mar+Apr+May+Jun+Jul+Aug+Sep+Oct+Nov+Dec,data = train)
...
multi_add_sea_model<-lm(Sales~Jan+Feb+Mar+Apr+May+Jun+Jul+Aug+Sep+Oct+Nov+Dec,data = train)
...
new_model <- lm(Sales~t+t_square+Jan+Feb+Mar+Apr+May+Jun+Jul+Aug+Sep+Oct+Nov+Dec,data=trakdata)
...
library(readXl)
test_data<-read.csv("D:/DS/Assignments/Assignments/Forecasting/CocaCola_Sales_Rawdata.csv")
View(test_data)
